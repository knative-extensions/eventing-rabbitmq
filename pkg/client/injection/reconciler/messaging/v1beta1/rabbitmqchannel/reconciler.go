...
import (
	...
	messagingv1beta1 "knative.dev/eventing-rabbitmq/pkg/client/listers/messaging/v1beta1"
...
// controller reconciling v1beta1.RabbitmqChannel.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1beta1.RabbitmqChannel. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1beta1.RabbitmqChannel) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1beta1.RabbitmqChannel.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1beta1.RabbitmqChannel. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1beta1.RabbitmqChannel) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1beta1.RabbitmqChannel if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1beta1.RabbitmqChannel.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1beta1.RabbitmqChannel) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1beta1.RabbitmqChannel if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1beta1.RabbitmqChannel.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1beta1.RabbitmqChannel) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1beta1.RabbitmqChannel) reconciler.Event
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister messagingv1beta1.RabbitmqChannelLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.RabbitmqChannels(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1beta1.RabbitmqChannel, desired *v1beta1.RabbitmqChannel) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.MessagingV1beta1().RabbitmqChannels(desired.Namespace)
		...
		updater := r.Client.MessagingV1beta1().RabbitmqChannels(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1beta1.RabbitmqChannel) (*v1beta1.RabbitmqChannel, error) {

	getter := r.Lister.RabbitmqChannels(resource.Namespace)
	...
	patcher := r.Client.MessagingV1beta1().RabbitmqChannels(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1beta1.RabbitmqChannel) (*v1beta1.RabbitmqChannel, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1beta1.RabbitmqChannel, reconcileEvent reconciler.Event) (*v1beta1.RabbitmqChannel, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
