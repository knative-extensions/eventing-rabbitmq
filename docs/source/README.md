...
| `adapterContainerName` \* | Custom name for the receive-adapter container. Defaults to `receive-adapter`. Useful for differentiating log streams across multiple RabbitmqSource instances. Note: cannot be changed after the resource is created. |
...
To set a custom container name for the receive adapter (useful for log differentiation across multiple sources):
...
spec:
  ...
  sink:
    ref:
      ...
      name: rabbitmq-source-sink
      namespace: default
  adapterContainerName: "my-rabbitmq-source-adapter"
```
