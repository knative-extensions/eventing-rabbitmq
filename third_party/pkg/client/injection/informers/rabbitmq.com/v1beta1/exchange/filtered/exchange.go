...
func withInformer(ctx context.Context) (context.Context, []controller.Informer) {
	...
	for _, selector := range labelSelectors {
		...
		inf := f.Rabbitmq().V1beta1().Exchanges()
...
func Get(ctx context.Context, selector string) v1beta1.ExchangeInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch knative.dev/eventing-rabbitmq/third_party/pkg/client/informers/externalversions/rabbitmq.com/v1beta1.ExchangeInformer with selector %s from context.", selector)
	}
	return untyped.(v1beta1.ExchangeInformer)
}
...
var _ rabbitmqcomv1beta1.ExchangeLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apisrabbitmqcomv1beta1.Exchange{}, 0, nil)
}

func (w *wrapper) Lister() rabbitmqcomv1beta1.ExchangeLister {
	return w
}
